...
def start():
    ...
    while True:
        if status == 1:         # healthy rover, and is not the leader.
            ...
            print("N" + str(rover_index) + " ROVER " + str(rover_index) + ": Cooling down for: " + str(task_require_time ) + " seconds")
            sleep(task_require_time)
                ...
                if info_type == 'health_report':
                    ...
                    if check_rover_temperature(data['temperature']) == 0:
                        ...
                        if temperature < 80:
                            ...
                            client.send(taskGenerator.generate_move_task(data['location_x'],data['location_y'])) # Send move+collect task to rover.

...
def do_task(task):
    ...
    if task['type'] == 'move+collect':
        ...
        msg = "ROVER1: Heading to Location (" + str(target_x) + ", " + str(target_y) + "), ETA: " + str(task_require_time) + " seconds"
        print(msg)
...
def start():
    ...
    while True:
        if status == 1:         # healthy rover, and is not the leader.
            ...
            print("ROVER 1: Coolinig down for: " + str(task_require_time ) + "seconds")
            sleep(task_require_time)
...
def init():
    ...
    try:
        ...
        print(task)
        print("ROVER1: successfully recieved task")
        task_socket.close()
        return task

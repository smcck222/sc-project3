...
def do_task(task):
    ...
    if task['type'] == 'move+collect':
        ...
        target_x,target_y,temperature,status,task_require_time = s.do_rover_task(task,location_x,location_y,temperature,status)
        msg = "ROVER 1: Heading to Location (" + str(target_x) + ", " + str(target_y) + "), ETA: " + str(
...
def start():
    ...
    while True:
        if status == 1:         # healthy rover, and is not the leader.
            ...
            task_require_time = ((temperature-40)//s.decreasing_rate)    # Time to cool down = (diff in temp/cooling rate)/2
            print("ROVER 1: Cooling down for: " + str(task_require_time ) + "seconds")
            sleep(task_require_time)
